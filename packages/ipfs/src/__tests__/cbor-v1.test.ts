...
describe('V1 CBOR — canary CIDs (mirrors pulse-protocol-go/ipfs/cbor_v1_test.go)', () => {
  ...
  it('V1 EC consent — known CID canary', async () => {
    const block = marshalV1ConsentEc({ consent: canaryConsent, key1: canaryKey1, key2: canaryKey2 })
  ...
  it('V1 EC revoke — known CID canary', async () => {
    ...
    const block = marshalV1RevokeEc({ revoke: canaryRevokeEC, key1: canaryKey1, key2: canaryKey2, grantRef })
    const cid = await getCid(block)
  ...
  it('V1 PQ revoke — known CID canary', async () => {
    ...
    const block = marshalV1RevokePq({ revoke: canaryRevokePQ, keys: [canaryKey1, canaryKey2], grantRef })
    const cid = await getCid(block)
