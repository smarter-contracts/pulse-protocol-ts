...
import { PulsePurpose, PulsePQEncryptionResult, PulsePQEncryptionKey } from '@pulse-protocol/types'
import { pulseHashBytes, toHex } from './hash.js'
import { pulseHkdfKyber } from './hkdf.js'
import { contextHash } from './context.js'
import { pulseSeal, pulseOpen, pulseSealWithNewKey, AES_KEY_SIZE, AES_NONCE_SIZE } from './symmetric.js'
...
export function encryptPq(
  ...
  const { ciphertext: sealedData, aesKey, nonce: dataNonce } = pulseSealWithNewKey(
    plaintext, purpose, PQ_DATA_CIPHER_SUITE, recipientHash, ctx
  )
  ...
  const keys: PulsePQEncryptionKey[] = recipientPublicKeys.map((pubKey) => {
    ...
    const { key: wrapKey, nonce: wrapNonce } = pulseHkdfKyber(sharedSecret, encapsulatedKey, fingerprint, ctx)
    const encapsulatedDataKey = pulseSeal(
      keyPacket, wrapKey, wrapNonce,
      PulsePurpose.SymmetricKeyWrap, PQ_KEY_CIPHER_SUITE,
      fingerprint, ctx, pulseHashBytes(encapsulatedKey)
    )
...
export function decryptPq(
  ...
  const { key: wrapKey, nonce: wrapNonce } = pulseHkdfKyber(
    sharedSecret, keyEntry.encapsulatedKeyKey, myFingerprint, ctx
  )

  // Unwrap the data key + nonce
  const keyPacket = pulseOpen(
    keyEntry.encapsulatedDataKey, wrapKey, wrapNonce,
    PulsePurpose.SymmetricKeyWrap, PQ_KEY_CIPHER_SUITE,
    myFingerprint, ctx, pulseHashBytes(keyEntry.encapsulatedKeyKey)
  )
  ...
  return pulseOpen(result.sealedData, dataAesKey, dataNonce, purpose, PQ_DATA_CIPHER_SUITE, recipientHash, ctx, transcriptHash)
}
