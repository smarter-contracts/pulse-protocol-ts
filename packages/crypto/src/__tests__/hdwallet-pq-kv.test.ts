...
import { masterKeyFromSeed, deriveNode, pulsePath, derivePqKeyPair, pqKeyFingerprint } from '../hdwallet.js'
...
describe('HD Wallet — NIST ML-KEM-768 known values', () => {
  ...
  describe('PQ consent key derivation (purpose 9)', () => {
    it('Alice PQ consent node priv/pub keys', () => {
      ...
      const node = deriveNode(alice, otherParty, chainId, consentNumber, PulsePurpose.PQDeriveConsent)
      expect(toHex(node.privateKey!)).toBe('7d3607e752508cf9e8e3188ef8db8890f5d7aac618865c9d4a52460f76d32a6b')
      expect(toHex(node.publicKey!)).toBe('02704fab5d7282c0e6719bcc762f2e45d481351741598483900b8e4c429d11e2b3')
    })

    it('Bob PQ consent node priv/pub keys', () => {
      const bob = masterKeyFromSeed(bobSeed)
      const node = deriveNode(bob, otherParty, chainId, consentNumber, PulsePurpose.PQDeriveConsent)
      expect(toHex(node.privateKey!)).toBe('af988f9d0ab2f603851d56d20bd0eb3749814dd44bfd2d5d638f900cee0be74b')
      expect(toHex(node.publicKey!)).toBe('03f7a0062f05bc7fc1f9187d80e22b72962195f198955a49ac0e2d86798ffb1db8')
    })

    it('Alice ML-KEM-768 consent public key fingerprint', () => {
      const alice = masterKeyFromSeed(aliceSeed)
      const { publicKey } = derivePqKeyPair(alice, otherParty, consentNumber, chainId, PulsePurpose.PQDeriveConsent)
    ...
    it('Bob ML-KEM-768 consent public key fingerprint', () => {
      const bob = masterKeyFromSeed(bobSeed)
      const { publicKey } = derivePqKeyPair(bob, otherParty, consentNumber, chainId, PulsePurpose.PQDeriveConsent)
  ...
  describe('PQ revoke key derivation (purpose 10)', () => {
    it('Alice PQ revoke node priv/pub keys', () => {
      const alice = masterKeyFromSeed(aliceSeed)
      const node = deriveNode(alice, otherParty, chainId, consentNumber, PulsePurpose.PQDeriveRevoke)
      expect(toHex(node.privateKey!)).toBe('923b9ee0e2e2ef45d8c5b8f91ac02c4be9bf6982d71ab5751c928021abba52af')
      expect(toHex(node.publicKey!)).toBe('03f3bb3680cfaec89a75fd0e99e8a20737e40f644f6c3243a8f058fd92ecc53aa4')
    })
    ...
    it('Bob PQ revoke node priv/pub keys', () => {
      ...
      expect(toHex(node.privateKey!)).toBe('8162d312cd9f751121b09ddd34b3d4f57ba056c73cd9c9d1c83367e3d4372be8')
      expect(toHex(node.publicKey!)).toBe('03f8998a3047c33e613107cfe9d68d142000c25e9e2efeb344ad4319eefb169545')
    })

    it('Alice ML-KEM-768 revoke public key fingerprint', () => {
      const alice = masterKeyFromSeed(aliceSeed)
      const { publicKey } = derivePqKeyPair(alice, otherParty, consentNumber, chainId, PulsePurpose.PQDeriveRevoke)
    ...
    it('Bob ML-KEM-768 revoke public key fingerprint', () => {
      const bob = masterKeyFromSeed(bobSeed)
      const { publicKey } = derivePqKeyPair(bob, otherParty, consentNumber, chainId, PulsePurpose.PQDeriveRevoke)
  ...
  describe('Revoke notary EC encryption (purpose 4)', () => {
    it('revoke notary sealed data matches known value', async () => {
      ...
      const alice = masterKeyFromSeed(aliceSeed)
      const aliceNode = deriveNode(alice, otherParty, chainId, consentNumber, PulsePurpose.EncryptRevokeNotaryBlock)
      ...
      const result = encryptEcdh(
        revokeNotaryData, contractAddress, aliceNode.privateKey!, notaryPub,
        PulsePurpose.EncryptRevokeNotaryBlock, chainId, consentNumber
      )
  ...
  describe('PQ consent encrypt/decrypt round-trip', () => {
    it('Alice encrypts with PQ keys for both parties, both can decrypt', async () => {
      ...
      const aliceKem = derivePqKeyPair(alice, otherParty, consentNumber, chainId, PulsePurpose.PQDeriveConsent)
      const bobKem = derivePqKeyPair(bob, otherParty, consentNumber, chainId, PulsePurpose.PQDeriveConsent)

      const plaintext = new TextEncoder().encode('pq consent payload for round-trip test')

      const result = encryptPq(
        plaintext, contractAddress,
        [aliceKem.publicKey, bobKem.publicKey],
        PulsePurpose.PQDeriveConsent, chainId, consentNumber
      )
      ...
      const decryptedAlice = decryptPq(
        result, contractAddress, aliceKem.secretKey, aliceKem.publicKey,
        PulsePurpose.PQDeriveConsent, chainId, consentNumber
      )
      ...
      const decryptedBob = decryptPq(
        result, contractAddress, bobKem.secretKey, bobKem.publicKey,
        PulsePurpose.PQDeriveConsent, chainId, consentNumber
      )
    ...
    it('Alice encrypts revoke with PQ keys, both can decrypt', async () => {
      ...
      const result = encryptPq(
        plaintext, contractAddress,
        [aliceKem.publicKey, bobKem.publicKey],
        PulsePurpose.PQDeriveRevoke, chainId, consentNumber
      )
